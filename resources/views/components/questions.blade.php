        ...
        <div class="max-w-7xl mx-auto">
            <ul class="text-gray-800 dark:text-gray-200">
                @foreach ($questions as $key => $question)
                    <li class="mb-24 px-10 pt-10 pb-14 bg-white dark:bg-neutral-800 shadow-sm rounded-lg">
                        ...
                        <div class="{{ isset($question->answer) ? 'pb-16' : '' }}">
                            @isset($question->answer->content)
                                <p class="pb-6 font-thin">
                                    class="py-2">{{ $question->answer->user->id === Auth::user()->id ? 'Yo' : $question->answer->user->name }}
                                </p>
                            @endisset
                            <p class="text-lg">
                        ...
                        <div class="flex">
                            @if ($question->user_id === Auth::user()->id)
                                <form action="{{ route('question.edit', $question) }}" method="POST" class="pt-12">
                                    @csrf
                                    @method('POST')
                                    <x-primary-button
